...
class JSXScanner extends text_scanner_1.TextScanner {
    ...
    async scanSelector() {
        ...
        if (match) {
            if (match.groups.identifier === '#' || match.groups.identifier === '.') {
                return simple_selector_1.SimpleSelector.create(match.groups.identifier + match.text, match.index, this.document);
            }
            else {
                return simple_selector_1.SimpleSelector.create(match.text, match.index, this.document);
            }
    ...
    async scanCSSModule(moduleName, moduleProperty, wordLeftOffset) {
        ...
        if (modulePath) {
            ...
            if (fullPath) {
                return simple_selector_1.SimpleSelector.create('.' + moduleProperty, wordLeftOffset, this.document, vscode_uri_1.URI.file(fullPath).toString());
            }
        }
        return simple_selector_1.SimpleSelector.create('.' + moduleProperty, wordLeftOffset, this.document);
    }
    /** Parse `import ...`. */
    ...
    async scanForImportPath() {
        ...
        if (match) {
            ...
            if (importPath) {
                ...
                let extension = (0, file_1.getPathExtension)(importPath);
                if (this.supportedLanguages.includes(extension)) {
