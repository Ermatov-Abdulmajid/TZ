...
connection.onInitialized(() => {
    ...
    if (configuration.enableFindAllReferences) {
        connection.onReferences(helpers_1.console.logListQuerierExecutedTime(server.findReferences.bind(server), 'reference'));
    }
});
...
class CSSNavigationServer {
    ...
    async findDefinitions(positionParams) {
        ...
        if (locations) {
            locations.sort((a, b) => {
                ...
                return (0, utils_1.getLongestCommonSubsequenceLength)(bPath, documentIdentifier.uri) - (0, utils_1.getLongestCommonSubsequenceLength)(aPath, documentIdentifier.uri);
            });
        }
        return locations?.map(l => {
            return vscode_languageserver_1.Location.create(l.targetUri, l.targetRange);
        }) || null;
    }
    ...
    async provideHTMLDocumentCompletion(document, position) {
        ...
        if (selector.importURI) {
            ...
            if (cssService) {
                ...
                return (0, utils_1.formatLabelsToCompletionItems)(labels, selector.startIndex, completeLength, document);
            }
            else {
                return null;
            }
    ...
    async provideCSSDocumentCompletion(document, position) {
        ...
        if (selectorResults.raw === '.' || selectorResults.raw === '#') {
            ...
            let items = (0, utils_1.formatLabelsToCompletionItems)(labels, selectorResults.startIndex, completeLength, document);
        ...
        // Has parent, must be processed to remove prefix.
            ...
            for (let selector of selectorResults.selectors) {
                ...
                // Note the complete label includes identifier.
                let completeLength = selectorResults.raw.length;
                let items = (0, utils_1.formatLabelsToCompletionItems)(labels, selector.startIndex, completeLength, document);
                completionItems.push(...items);
            }
        }
